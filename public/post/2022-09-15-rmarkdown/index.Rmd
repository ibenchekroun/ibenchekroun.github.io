...
The goal of this post is to use my amateur web scraping and more-than-amateur data analysis skills to explore the following question:<font size="5"> <strong> are specific kinds of movies more likely to encounter hate on IMDb than others? </strong> </font> Are movies with a woman lead more likely to be hated on than movies with a non-woman lead? How about if the two leads are women? If it was (co-)written by a woman, or (co-)directed by a woman? I'll also look into non-gender variables obtainable on IMDb. The full list of variables considered, and their definitions, is displayed in the [Glossary](Glossary).   
...
<!-- <font size = "1.5"> Note: An alternative analysis could be to instead determine the average degree of Hate for each specific kind of movie. For instance, we could take the mean or median proportion of 1/10 reviews for woman-led films and compare to the same metric but for non-woman-led films. My belief is that the most extreme cases of IMDb hate, ie the 100 most hated films, are more likely to reveal user prejudice than the average hate across all movies. </font>  -->
...
Maybe you're still not convinced. If it helps, below is a chart of the Metascore ratings of the 2400+ movies released between 2000 and 2019 that received 50K+ IMDb ratings; please <b> feel free to hover over movies with high, medium, and low Metascores </b>. Or in the table below (Table 1), <b> you can look up the title of any movie in the Search bar to see its Metascore rating </b>. </font>
...
The top 'Hated' films, ie the 100 most hated films for each Metascore group, are determined. The proportion of 1/10 IMDb ratings for Hated movies ranges from      
...
get_chisq_p_value <- function(most_hated_row_ls){
  # most_hated_row <- most_hated_wider_df[1,]
...
most_hated_wider_df$metascore_group = with(most_hated_wider_df, ifelse(Metascore_Score_Meaning %>% str_detect("Favorable"), 2, ifelse(Metascore_Score_Meaning %>% str_detect("Mixed"), 1, 3)))
...
* Unfavourable films show the <strong> least </strong> correlation between IMDb hate and any of the movie traits. This supports the idea that critically disliked films <i> earn </i> their 1/10 ratings on IMDb; the hate is based on actual movie quality rather than any prejudice. 
* Mixed films show the <strong> most </strong> correlations between IMDb hate and the considered movie traits. Perhaps when a movie is not great - as is the case for Mixed movies - it enables audiences to act on their biases and leads to harsher, more exaggerated ratings.
...
```
\vspace{-5truemm}      
<i> <font size = "1.5">  The bar plot above shows the different proportions of Hated movies for each movie trait, within each Metascore group. The dark bars represent movies that have the given trait, and light bars do not have it. For instance, the dark green bar at the very top right represents the proportion of Favourable horror movies that are Hated (28.05%), and the light green bar to the very left of it is the proportion of Favourable non-horror movies that are Hated (7.86%). Note: movie traits representing fewer than 30 movies in our sample are not shown.  </font>     </i>

<!-- ### Alternative approach: Finding mean/median proportion of 1/10 ratings per kind of movie -->

<!-- ```{r, echo = FALSE} -->
<!-- get_by_trait_average_hate <- function(movie_trait, all = FALSE){ -->
<!--   all_movies_with_ratings_df50K_hate_all <- all_movies_with_ratings_df50K_hate %>%  -->
<!--     group_by(Metascore_Score_Meaning, all_movies_with_ratings_df50K_hate[[movie_trait]]) %>%  -->
<!--     summarise(num_of_movies = n(),  -->
<!--               hate_mean = mean(Percentage_1_numeric),  -->
<!--               hate_median = median(Percentage_1_numeric) -->
<!--     ) %>% mutate(movie_trait = movie_trait) -->
<!--   colnames(all_movies_with_ratings_df50K_hate_all)[2] <- "bin" -->
<!--   all_movies_with_ratings_df50K_hate_all$rownames <- with(all_movies_with_ratings_df50K_hate_all, ifelse(bin == 0, paste0("Non-", movie_trait), movie_trait)) -->
<!--   all_movies_with_ratings_df50K_hate_all -->
<!-- } -->

<!-- get_ttest_p_value <- function(average_hate_row_ls){ -->
<!--   # average_hate_row_ls <- average_hate_wider_df[1,] -->
<!--   movie_trait = average_hate_row_ls$movie_trait -->
<!--   grp = average_hate_row_ls$Metascore_Score_Meaning -->
<!--   movies = all_movies_with_ratings_df50K_hate %>% filter(Metascore_Score_Meaning == grp) -->

<!--   p_value <- (with(movies, t.test(Percentage_1_numeric[movies[,movie_trait] == 1], Percentage_1_numeric[movies[,movie_trait] == 0])))$p.value -->
<!--   average_hate_row_ls$p_value <- round(p_value, 3) -->
<!--   average_hate_row_ls -->
<!-- } -->

<!-- average_hate_df <- get_by_trait_average_hate("Lead1_female") %>%   -->
<!-- # ## Women-led -->
<!-- # bind_rows(, -->

<!-- ## Two-men-led -->
<!-- bind_rows(get_by_trait_average_hate("Leads_male_male"), -->

<!--           get_by_trait_average_hate("Leads_female_female"), -->

<!-- ## Horror -->
<!-- get_by_trait_average_hate("Horror_bin"), -->

<!-- ## Drama -->
<!-- get_by_trait_average_hate("Drama_bin"), -->

<!-- ## Comedy -->
<!-- get_by_trait_average_hate("Comedy_bin"), -->

<!-- ## Action -->
<!-- get_by_trait_average_hate("Action_bin"), -->

<!-- ## Franchise -->
<!-- get_by_trait_average_hate("Franchise_bin"), -->

<!-- ## Woman Writer -->
<!-- get_by_trait_average_hate("woman_writer_bin"), -->

<!-- ## Woman Director -->
<!-- get_by_trait_average_hate("woman_director_bin"), -->

<!-- ## Auteur -->
<!-- get_by_trait_average_hate("Auteur_bin") -->

<!-- ) -->
<!-- # most_hated_wider_df -->

<!-- average_hate_wider_df <- average_hate_df %>% select(-rownames) %>% pivot_wider(names_from = bin, values_from = c(num_of_movies, hate_mean, hate_median)) -->

<!-- average_hate_wider_df$metascore_group = with(average_hate_wider_df, ifelse(Metascore_Score_Meaning %>% str_detect("Favorable"), 2, ifelse(Metascore_Score_Meaning %>% str_detect("Mixed"), 1, 3))) -->

<!-- average_hate_wider_ls <- split(average_hate_wider_df, seq(nrow(average_hate_wider_df))) -->
<!-- average_hate_wider_df <- average_hate_wider_ls %>% map_df(get_ttest_p_value) %>% select(-c(ends_with("-1"))) -->

<!-- # TTEST_PVALUE(average_hate_wider_df[1,]) -->

<!-- sample_size_threshold <- 30 -->
<!-- average_hate_wider_df$hate_mean_0_plot <- with(average_hate_wider_df, ifelse(num_of_movies_0 < sample_size_threshold | num_of_movies_1 < sample_size_threshold, 0, round(hate_mean_0, 4 ))) -->


<!-- average_hate_wider_df$hate_mean_1_plot <- with(average_hate_wider_df, ifelse(num_of_movies_0 < sample_size_threshold | num_of_movies_1 < sample_size_threshold, 0, round(hate_mean_1, 4) )) -->

<!-- average_hate_wider_df <- average_hate_wider_df %>% merge(movie_trait_df, all.x = TRUE) -->
<!-- average_hate_wider_df$Delta <- with(average_hate_wider_df, hate_mean_1 - hate_mean_0) -->
<!-- delta_table <- average_hate_wider_df %>%  -->
<!--     filter(Metascore_Score_Meaning == metascore_groups[2]) %>% arrange( Delta) %>% as.data.frame() %>% select(movie_trait_readable, Delta) %>% unique() -->

<!-- average_hate_wider_df$movie_trait_readable <- factor(average_hate_wider_df$movie_trait_readable, levels = delta_table$movie_trait_readable) -->

<!-- ``` -->

<!-- ```{r, include = FALSE} -->

<!-- fig1 <- plot_ly(data= average_hate_wider_df %>% filter(Metascore_Score_Meaning == metascore_groups[2]), x = ~movie_trait_readable, y = ~hate_mean_0_plot, type = 'bar', name = "Doesn't have movie trait", marker = list(color = "#96ceb4"), -->
<!--                  text = ~num_of_movies_0, -->
<!--                 # text2 = ~p_value , -->
<!--                 textposition = "none", -->
<!--                 hovertext = ~ paste0("<br><b>Non-", `movie_trait_readable`, "</b><br>",  -->
<!--                           "Number of movies: ", num_of_movies_0, "<br>", -->
<!--                           "<b>", hate_mean_0_plot, "</b> are among the 100 most hated <i>Favourable</i> films <br>", -->
...
<!-- fig1 <- fig1 %>% add_trace(y = ~hate_mean_1_plot, name = 'Has movie trait', marker = list(color = "#86af49"), -->
<!--                  text = ~num_of_movies_1, -->
<!--                 textposition = "none", -->
<!--                 hovertext = ~ paste0("<br><b>", `movie_trait_readable`, "</b><br>",  -->
<!--                           "Number of movies: ", num_of_movies_1, "<br>", -->
<!--                           "<b>", hate_mean_1_plot, "</b> are among the 100 most hated <i>Favourable</i> films <br>", -->
<!--                           'T-test p-value:', p_value -->
<!-- ), -->
<!-- hovertemplate = "%{hovertext}<extra></extra>") -->
...
<!-- fig2 <- plot_ly(data= average_hate_wider_df %>% filter(Metascore_Score_Meaning == metascore_groups[1]), x = ~movie_trait_readable, y = ~hate_mean_0_plot, type = 'bar', name = 'No', marker = list(color = '#EEDC82'), -->
<!--                  text = ~num_of_movies_0, -->
<!--                 textposition = "none", -->
<!--                 hovertext = ~ paste0("<br><b>Non-", `movie_trait_readable`, "</b><br>",  -->
<!--                           "Number of movies: ", num_of_movies_0, "<br>", -->
<!--                           "<b>", hate_mean_0_plot, "</b> are among the 100 most hated <i>Mixed</i> films <br>", -->
<!--                           'T-test p-value:', p_value -->
<!-- ), -->
<!-- hovertemplate = "%{hovertext}<extra></extra>") -->

<!-- fig2 <- fig2 %>% add_trace(y = ~hate_mean_1_plot, name = 'Yes', marker = list(color = '#DAA520'), -->
<!--                  text = ~num_of_movies_1, -->
<!--                 textposition = "none", -->
<!--                 hovertext = ~ paste0("<br><b>", `movie_trait_readable`, "</b><br>",  -->
<!--                           "Number of movies: ", num_of_movies_1, "<br>", -->
<!--                           "<b>", hate_mean_1_plot, "</b> are among the 100 most hated <i>Mixed</i> films <br>", -->
<!--                           'T-test p-value:', p_value -->
<!-- ), -->
<!-- hovertemplate = "%{hovertext}<extra></extra>") -->
...
<!--                title = "Top 100 ", -->
<!--                xaxis = list(title = "Movie Trait?", showticklabels = FALSE), -->
<!--                yaxis = list(title = '<b> Mean hate </b> \n Mixed' #, -->
<!--                             # showgrid = FALSE -->
<!--                             , range=c(0,5), -->
<!--                             tickfont = list(color = '#b2b2b2')),  -->
<!--                barmode = 'group' #, -->
...
<!-- fig3 <- plot_ly(data= average_hate_wider_df %>% filter(Metascore_Score_Meaning == metascore_groups[3]), x = ~movie_trait_readable, y = ~hate_mean_0_plot, type = 'bar', name = 'No', marker = list(color = '#f7cac9'), -->
<!--                  text = ~num_of_movies_0, -->
<!--                 textposition = "none", -->
<!--                 hovertext = ~ paste0("<br><b>Non-", `movie_trait_readable`, "</b><br>",  -->
<!--                           "Number of movies: ", num_of_movies_0, "<br>", -->
<!--                           "<b>", hate_mean_0_plot, "</b> are among the 100 most hated <i>Unfavourable</i> films <br>", -->
<!--                           'T-test p-value:', p_value -->
<!-- ), -->
<!-- hovertemplate = "%{hovertext}<extra></extra>") -->

<!-- fig3 <- fig3 %>% add_trace(y = ~hate_mean_1_plot, name = 'Yes', marker = list(color = '#bc5a45'), -->
<!--                  text = ~num_of_movies_1, -->
<!--                 textposition = "none", -->
<!--                 hovertext = ~ paste0("<br><b>", `movie_trait_readable`, "</b><br>",  -->
<!--                           "Number of movies: ", num_of_movies_1, "<br>", -->
<!--                           "<b>", hate_mean_1_plot, "</b> are among the 100 most hated <i>Unfavourable</i> films <br>", -->
<!--                           'T-test p-value:', p_value -->
<!-- ), -->
<!-- hovertemplate = "%{hovertext}<extra></extra>") -->

<!-- fig3 <- fig3 %>% layout( -->
<!--                title = "<b> Mean hate, by Movie Trait </b> - <i> \n grouped by Metacritics Reception</i> \n \n", -->
<!--                xaxis = list(title = "Movie Trait", tickangle = 45), -->
<!--                yaxis = list(title = ' \n Unfavourable' #, -->
<!--                             # showgrid = FALSE -->
<!--                             , range=c(0,8), -->
...
<!-- ### Regression -->
<!-- ```{r, include = FALSE} -->
<!-- lm_favourable <- with(all_movies_with_ratings_df50K_hate %>% filter(Metascore_Score_Meaning == metascore_groups[2]), glm(Most_hated_bin ~ Horror_bin + Lead1_female + Auteur_bin, family = "binomial")) -->
<!-- summary(lm_favourable) -->

<!-- all_movies_with_ratings_df50K_hate$pred_prob <- with(all_movies_with_ratings_df50K_hate, ifelse(Metascore_Score_Meaning == metascore_groups[2], fitted(lm_favourable, all_movies_with_ratings_df50K_hate  %>% filter(Metascore_Score_Meaning == metascore_groups[2])), NA)) -->


<!-- lm_mixed <- with(all_movies_with_ratings_df50K_hate %>% filter(Metascore_Score_Meaning == metascore_groups[1]), glm(Most_hated_bin ~ Horror_bin + Lead1_female + Auteur_bin + woman_writer_bin + Franchise_bin, family = "binomial")) -->
<!-- summary(lm_mixed) -->

<!-- lm_unfavourable <- with(all_movies_with_ratings_df50K_hate %>% filter(Metascore_Score_Meaning == metascore_groups[3]), glm(Most_hated_bin ~ Auteur_bin, family = "binomial")) -->
<!-- summary(lm_unfavourable) -->

<!-- all_movies_with_ratings_df50K_hate$pred_prob <- with(all_movies_with_ratings_df50K_hate, ifelse(Metascore_Score_Meaning == metascore_groups[1], fitted(lm_mixed, all_movies_with_ratings_df50K_hate  %>% filter(Metascore_Score_Meaning == metascore_groups[1])), pred_prob)) -->



<!-- all_movies_with_ratings_df50K_hate$pred_prob <- with(all_movies_with_ratings_df50K_hate, ifelse(Metascore_Score_Meaning == metascore_groups[3], fitted(lm_unfavourable, all_movies_with_ratings_df50K_hate  %>% filter(Metascore_Score_Meaning == metascore_groups[3])), pred_prob))                                                                                    -->

<!-- all_movies_with_ratings_df50K_hate$Most_hated_bin_pred <- with(all_movies_with_ratings_df50K_hate, ifelse(pred_prob >= 0.4, 1, 0)) -->

<!-- with(all_movies_with_ratings_df50K_hate,  -->
<!--      table(Most_hated_bin, Most_hated_bin_pred) -->
<!-- ) -->
<!-- ``` -->
